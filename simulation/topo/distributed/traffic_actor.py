...
class TrafficActor:
	...
	def _start_traffic_to_target_process(self, hid, target_id, flow_type, to_schedule=False,
	                                  cnt_process=4):
	...
	def act(self, mode: str):
		...
		if self.prev_mode == "small":
			if mode == "large":
				debug("current mode:{}".format(self.cnt % 6 + 1))
				if self.cnt % 6 == 0:
					for hid in [6]:
						if hid in self.hostids:
							self._start_traffic_to_target_list(hid, [12,13,14,15], "video",
							                                   True)
				if self.cnt % 6 == 1:
					for hid in [6]:
						if hid in self.hostids:
							self._start_traffic_to_target_list(hid, [12,13,14,15],
							                                   "video", True)
				if self.cnt % 6 == 2:
					for hid in [6]:
						if hid in self.hostids:
	...
	def act1(self, configStr: str):
		...
		for item in arr:
			...
			dst = tmp[1]
			cnt = t[1]
			self._start_traffic_to_target_process(int(src), int(dst), "video", False, int(cnt))
		debug("config traffic done*********")
